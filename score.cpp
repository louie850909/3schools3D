...
static clock_t					g_StartTime;				// 開始時間
...
{
	clock_t nowTime = clock();	// 現在時間
	double time = (double)(nowTime - g_StartTime) / CLOCKS_PER_SEC;	// 経過時間
	if (time >= 1.0f)
	{
		g_StartTime = nowTime;	// 開始時間
		g_Time++;	// 時間を加算
